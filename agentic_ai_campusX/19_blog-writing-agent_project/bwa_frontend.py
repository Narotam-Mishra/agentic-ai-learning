...
# blog writing agent frontend using streamlit
...
# Keep all relative blog/image paths anchored to this application's directory,
# regardless of the directory from which Streamlit was launched.
    ...
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as z:
        ...
        if images_dir.exists() and images_dir.is_dir():
            for p in images_dir.rglob("*"):
                if p.is_file():
                    z.write(p, arcname=str(Path(images_dir.name) / p.relative_to(images_dir)))
    return buf.getvalue()
...
def images_zip(images_dir: Path) -> Optional[bytes]:
    ...
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as z:
        for p in images_dir.rglob("*"):
            if p.is_file():
                z.write(p, arcname=str(p.relative_to(images_dir)))
    return buf.getvalue()
...
def list_past_blogs() -> List[Path]:
    """
    Returns .md files in the application directory, newest first.
    Filters out obvious non-blog markdown files if needed.
    """
    files = [p for p in BASE_DIR.glob("*.md") if p.is_file()]
...
with st.sidebar:
    ...
    if not past_files:
        st.caption(f"No saved blogs found (*.md in {BASE_DIR}).")
...
if st.session_state.get("loaded_blog_name"):
    st.success(
        f"Loaded {st.session_state['loaded_blog_name']}. "
        "Open the 📝 Markdown Preview tab to view it."
    )
...
if run_btn:
    ...
    st.session_state.pop("loaded_blog_name", None)
...
if out:
    ...
    with tab_preview:
        ...
        if not final_md:
            ...
            bundle = bundle_zip(final_md, md_filename, BLOG_IMAGES_DIR)
            st.download_button(
    ...
    with tab_images:
        ...
        if not specs and not images_dir.exists():
            ...
            if images_dir.exists():
                ...
                if not files:
                    st.warning("blog_images/ exists but is empty.")
                else:
